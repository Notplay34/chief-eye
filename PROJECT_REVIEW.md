...
> Документ-аудит на конкретный момент времени.
> Использовать как вспомогательный обзор, а не как главный источник текущего состояния проекта.
...
| Проблема | Где |
|----------|-----|
| В репозитории оставался слой `django_backend/` | Занимал место и путал структуру проекта до удаления |

**Рекомендация:** не возвращать второй backend без явного архитектурного решения и отдельного плана миграции.  
**Статус:** слой `django_backend/` удалён из репозитория; основной контур остаётся в `backend/app` (FastAPI + SQLAlchemy).
...
1. **Проверить и удалить мёртвый код**  
   Задача выполнена для `django_backend/`; дальше смотреть на устаревшие документы и дубли в frontend/backend.
